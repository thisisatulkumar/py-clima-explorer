...
def dataset_comparison_page():
    # ── CSS ──────────────────────────────────────────────────────────────────
    st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500;600&display=swap');
    :root {
        --bg:#080c14; --bg2:#0e1420; --bg3:#141b2d; --border:#1e2d4a;
        --col-a:#00e5ff; --col-b:#ff6b35; --col-neu:#c8d4f0; --col-dim:#5a6a8a;
        --pos:#22d3a5; --neg:#f43f5e;
    }
    .page-hero { background:linear-gradient(135deg,#0e1420,#141b2d); border:1px solid var(--border);
        border-radius:16px; padding:2rem 2.5rem; margin-bottom:1.5rem; }
    .page-title { font-family:'Bebas Neue',sans-serif; font-size:3.8rem; letter-spacing:3px; margin:0;
    ...
    .ds-label-a { color:var(--col-a); } .ds-label-b { color:var(--col-b); }
    ...
    .sec-head { font-family:'Bebas Neue',sans-serif; font-size:1.5rem; letter-spacing:2px; color:var(--col-neu);
        border-bottom:1px solid var(--border); padding-bottom:0.4rem; margin:2rem 0 1rem 0; }
    .insight-card { background:var(--bg3); border:1px solid var(--border); border-radius:12px; padding:1rem 1.2rem; margin-bottom:0.5rem; }
    ...
    .insight-val-pos { color:var(--pos); font-weight:600; } .insight-val-neg { color:var(--neg); font-weight:600; }
    .insight-val-a { color:var(--col-a); font-weight:600; } .insight-val-b { color:var(--col-b); font-weight:600; }
    .verdict { background:linear-gradient(135deg,rgba(124,58,237,0.12),rgba(124,58,237,0.05));
        border:1px solid rgba(124,58,237,0.4); border-radius:14px; padding:1.4rem 1.8rem; margin-top:1.5rem; }
    ...
    variable = st.selectbox("🌡️ Climate Variable", ["Temperature", "Precipitation", "Wind Speed"], key="climate_variable_select")
    ...
    run = st.button("⚡ RUN COMPARISON", use_container_width=True, key="run_comparison_button",
                    type="primary")

    if not run:
        st.markdown("""
        <div style='text-align:center;padding:3rem 2rem;color:#5a6a8a;font-family:"DM Sans",sans-serif;'>
          <div style='font-size:0.9rem;color:#c8d4f0;'>👆 Configure datasets above then hit RUN COMPARISON</div>
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    c1, c2 = st.columns(2)
    with c1: st.plotly_chart(chart_seasonal(r), use_container_width=True)
    with c2: st.plotly_chart(chart_radar(r), use_container_width=True)

    st.markdown("<div class='sec-head'>🌐 SPATIAL & DISTRIBUTION</div>", unsafe_allow_html=True)
    c3, c4 = st.columns(2)
    with c3: st.plotly_chart(chart_zonal(r), use_container_width=True)
    with c4: st.plotly_chart(chart_box(r), use_container_width=True)
    c5, c6 = st.columns(2)
    with c5: st.plotly_chart(chart_histogram(r), use_container_width=True)
    with c6: st.plotly_chart(chart_cdf(r), use_container_width=True)

    st.markdown("<div class='sec-head'>🥧 PROPORTIONAL INSIGHTS</div>", unsafe_allow_html=True)
    c7, c8 = st.columns(2)
    with c7: st.plotly_chart(chart_stats_pie(r), use_container_width=True)
    with c8: st.plotly_chart(chart_seasonal_pie(r), use_container_width=True)

    fig_score, wins_a, wins_b, score_details = chart_stats_donut(r)
    cs1, cs2 = st.columns([1, 1.4])
    with cs1: st.plotly_chart(fig_score, use_container_width=True)
    with cs2:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div style="font-family:Bebas Neue,sans-serif;font-size:1.1rem;letter-spacing:1.5px;color:#c8d4f0;margin-bottom:0.8rem;">METRIC BREAKDOWN</div>', unsafe_allow_html=True)
        for icon, detail in zip(["🌡️", "📉", "🚀", "↔️"], score_details):
            st.markdown(f'<div style="background:#141b2d;border:1px solid #1e2d4a;border-radius:10px;padding:0.7rem 1rem;margin-bottom:0.5rem;font-family:DM Sans;font-size:0.88rem;color:#c8d4f0;">{icon} {detail}</div>', unsafe_allow_html=True)
    ...
    for i in range(0, len(insights), 2):
        ...
        for col, idx in [(ci1, i), (ci2, i + 1)]:
            if idx < len(insights):
                ins = insights[idx]
                col.markdown(f"<div class='insight-card'><div class='insight-icon'>{ins['icon']}</div><p class='insight-title'>{ins['title']}</p><p class='insight-body'>{ins['body']}</p></div>", unsafe_allow_html=True)

    winner = label_a if wins_a >= wins_b else label_b
    dm = abs(r["delta_mean"]); dt = abs(r["delta_trend"])
    ...
    <div class='verdict'>
      ...
      <p class='verdict-body'>
        ...
        Warming trends diverge by <b>{dt:.3f} {r["units"]}/decade</b>.
        {label_a} spans <b>{r["a"]["year_start"]}–{r["a"]["year_end"]}</b> ·
        {label_b} spans <b>{r["b"]["year_start"]}–{r["b"]["year_end"]}</b>.
      </p>
    </div>
    """, unsafe_allow_html=True)
